...
describe("integration", () => {
  ...
  it("should index Next.js docs", async () => {
...
`;
  ...
  it("should index Axum repo", async () => {
...
`;
  ...
  it("should index Temporal docs", async () => {
...
`;
  ...
  it("should index Auth0 docs", async () => {
...
repos:
  - name: auth0
    repo: https://github.com/auth0/docs
    sourcePath: articles
`;
    await writeFile(path.join(projectDir, "docpup.config.yaml"), config);
...
`;
    ...
    const result = await runDocpup(projectDir, ["--only", "repo-a"]);
    ...
    expect(await fileExists(indexB)).toBe(false);
  });
...
`;
  ...
  it("should handle repos with no markdown files gracefully", async () => {
    const projectDir = path.join(testDir, "no-md-test");
    await mkdir(projectDir, { recursive: true });
...
`;
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("0 files total");
  });
});
